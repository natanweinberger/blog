...
# Charsets, collations, and slow joins in MySQL

## The problem

I have two tables, `artist` and `song`. They look like this:
...
I'd like to join them to match each song to its artist.
...
This join executes as expected -- quick and easy.

## A step further

I gained access to a new data source for artists. It has all the same fields, but updates with new artists as soon as they register on recording platforms, even if they don't have songs published yet. So instead of having about details for 150,000 artists, I now have about 200,000.

Because all the fields are the same, I'm going to load this into a table called `artist_v2` and confirm my queries still work. I'll add a foreign key from `song` to the new `artist_v2` table to replicate the existing setup as closely as possible.
...
The results are correct, but the query took 3 seconds! That's way slower, something's off.
